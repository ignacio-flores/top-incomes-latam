...
for (i in 1:n_tables) {
  ...
  for (year in 1946:2019) {
    ...
    if (nrow(data) > 0) {
      ...
      # ---------------------- NEW: merge all Value Footnotes cols ---------------------- #
      # Find any column named "Value Footnotes" (possibly duplicated as ...12, ...13, etc.)
      fn_cols <- grep("^Value Footnotes(\\.\\.\\d+)?$", names(data), value = TRUE)
      
      if (length(fn_cols) > 0) {
        data <- data %>%
          rowwise() %>%
          mutate(`Value Footnotes` = {
            vals <- c_across(all_of(fn_cols)) |> as.character()
            vals <- vals[!is.na(vals) & nzchar(vals)]
            ids  <- unlist(strsplit(paste(vals, collapse = ","), ",", fixed = TRUE))
            ids  <- unique(ids[nzchar(ids)])
            if (length(ids) == 0) NA_character_ else paste(ids, collapse = ",")
          }) %>%
          ungroup()
        # we'll drop all "Value Footnotes*" columns later with your existing line
      }
      # ------------------------------------------------------------------------------- #
      
      # Merge to specific rows
      # If no footnote columns existed, create empty list to avoid pull() error
      if (!"Value Footnotes" %in% names(data)) {
        data_footnotes <- replicate(nrow(data), character(0), simplify = FALSE)
      } else {
        data_footnotes <- data %>% pull(`Value Footnotes`) %>%
          strsplit(split = ",", fixed = TRUE)
      }
      
      # Avoid shadowing outer loop index `i`
      for (ri in seq_len(nrow(data))) {
        ids <- data_footnotes[[ri]]
        if (length(ids) > 0 && !all(is.na(ids))) {
          j <- 1
          for (id in ids) {
            if (nrow(footnotes[footnotes$footnote_id == id, "footnote"]) == 1) {
              data[ri, paste0("footnote", j)] <- footnotes[footnotes$footnote_id == id, "footnote"]
              j <- j + 1
            }
          }
        }
      ...
      # Drop all "Value Footnotes*" columns (including the merged one after use)
